...
const App = () => {
  ...
  const handleAsk = async () => {
    ...
    try {
      ...
      const msg = typeof payload === "string" ? payload : (payload?.message || "⚠️ Error processing your request.");
      const botErr = { role: "bot", content: msg, time: new Date().toLocaleTimeString() };
        ...
        {chatLog.map((msg, idx) => (
          <div key={idx} style={{ marginBottom: 10, display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
            <div style={{ display: "flex", alignItems: "flex-end", maxWidth: "80%" }}>
              ...
              <div style={{ background: msg.role === "user" ? "#daf1fc" : "#f0f0f0", padding: 10, borderRadius: 10 }}>
        ...
        {loading && (
          <div style={{ display: "flex", justifyContent: "flex-start", marginBottom: 10 }}>
            <div style={{ display: "flex", alignItems: "flex-end", maxWidth: "80%" }}>
              ...
              <div style={{ background: "#f0f0f0", padding: 10, borderRadius: 10, fontStyle: "italic", fontSize: 14, display: "flex" }}>
